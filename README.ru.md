...
- **Нажатие vs. Удержание**
  - Назначьте два поведения для любой клавиши – одно для быстрого нажатия, второе для небольшого удержания. Лёгкое назначение клавиш с двойной ролью.

- **Мультинажатия**
  - Каждое нажатие это не просто выполнение назначенного действия, это путь к новым уникальным вложенным назначениям. Добавьте действие на любую последовательность нажатий.

- **Неограниченная вложенность**
  - Назначайте действие на любой глубине: хотите использовать пробел для ввода азбуки Морзе? Или добавить автокоррекцию длинного слова? Каждое нажатие это новый уровень, и новое поле для назначений для всех клавиш, без ограничения длины.
...
- **Тонкая настройка каждого нажатия**
  - Добавьте к любому назначению дополнительное действия при отжатии клавиши, настройте время срабатывания удержания или время ожидания следующей клавиши, укажите тип безвозвратного нажатия или моментального срабатывания без нарушения цепочки.
...
### 🔄 Первый запуск
1. **Инициализация раскладок**: при первом запуске приложения нажмите стандартное сочетание клавиш для смены раскладки (например, `Win + Space`), сменив их по кругу, чтобы скрипт узнал, какие раскладки вы используете.
2. **Окно клавиатуры**: после на экране появится стилизованная клавиатура с двумя списками – слоёв и аккордов, и дополнительными кнопками управления. Внимательно настройте внешний вид и поведение GUI через 🔧 в правом нижнем углу. (Дайте знать, если стандартный авторазмер окажется слишком большим)
...
- Дополнительные цветные индикаторы отображают изменённые настройки каждого назначения: в верхней части кнопки – для базового назначения; в нижней – для назначения по удержанию. В правом углу красным счётчиком или просто индикатором (меняется в настройках) отображено количество/наличие дочерних переходов. В левой части находятся индикаторы изменённых настроек нажатия. В порядке отображения:
...
#### **Переходы**
Нажатие и удержание физических клавиш клавиатуры переведут вас на соответствующий уровень назначений. Либо используйте мышь – левый клик для перехода с базовым нажатием, правый – с удержанием. После перехода вам откроются новые пункты для редактирования или очистки значений по текущему пути.

#### **Путь**
Стрелки над окном с раскладкой отображают путь до текущего вида (текущей таблицы назначений).
- Стрелками обозначен тип перехода – ➤ для базового нажатия, ▲ для удержания, и ▼ для аккорда. Если стрелку сопровождает цифра (как `2➤`) – значит данный переход был выполнен с соответствующим значением модификатора (это может быть как одиночный модификатор, так и сумма значений нескольких).
- Для возврата к любому из предыдущих уровней нажмите по соответствующей кнопке. Значение модификатора перехода этого уровня будет сохранено. Нажатие по кнопке текущего уровня сбросит все активные модификаторы.
...
### 📌 Встроенные слои
Несколько слоёв размещены сразу в репозитории, чтобы вы могли ознакомиться с использованием и возможностями, прежде чем назначите собственные:
- **Default**: 83 дополнительных символа расширенной пунктуации в буквенной части всего с одним модификатором. И неразрывный пробел со вторым.
...
- **Extra langs**: зачем использовать целую отдѣльную раскладку ради пары новых бўкв? (*зависит от раскладки*)
...
### 🎨 Первые назначения
Чтобы добавить или изменить назначение – нажмите на соответствующую клавишу на вашей клавиатуре или нажмите на неё мышью в интерфейсе. После нажатия, сверху справа от раскладки появится меню с панелями `Base` и `Hold`. `Base` отвечает за назначение при нажатии, `Hold` – при удержании. Нажмите на их кнопки, чтобы открыть форму для новых назначений.
...
- Моментальная активация – делает назначение независимым от дочерних переходов, выполняя действие сразу при отлове. Цепочка переходов при этом не теряется, и вы можете продолжать погружение.
...
- Изменённое время ожидания удержания – значение в мс, которое будет использовано для разделения tap/hold, вместо глобально заданного. **!** Это действует именно с базовыми назначениями. Другими словами, вы указываете базовому назначению, когда оно перейдёт в назначение от удержания.
...
> Кнопкам системных модификаторов может быть назначено только действие пользовательского модификатора, поэтому у них будут отстутствовать почти все упомянутые поля.

### ✨ Принцип действия

Когда вы нажимаете клавишу, скрипт проверяет, есть ли в текущей таблице переходов назначение для её нажатия и удержания.
  - Если есть только первое, оно сразу уходит в следующую проверку.
...
#### Переход
После определения назначения (базовое или от удержания) у него проверяется собственная, вложенная таблица переходов. Если она есть, и она не пустая, текущая таблица переходов заменяется новой, и скрипт ожидает следующую клавишу по указанному или глобальному значению таймера, без выполнения действия текущего назначения. Если у назначения добавлена опция `Instant`, действие будет выполнено сразу же, а таблица так же перейдёт к следующему узлу, с ожиданием новой клавиши.
  - Если за заданный таймер не было нового нажатия, последнее действие выполняется (здесь и далее – если не было выполнено ранее при `Instant`), а таблица переходов возвращается к стартовой, если не указана опция `Irrevocable`.
  - Если последующее нажатие отсутствует в текущей таблице переходов, выполняется последнее действие, а новое нажатие обрабатывается от корневой таблицы переходов.
  - Если новое нажатие присутствует в таблице переходов – оно начинает с первого пункта этого параграфа (с проверки базового назначения, от удержания, и далее).

#### Возврат к началу
Каждый раз, когда вы доходите до последнего элемента цепочки (листа), либо если цепочка прервалась таймером, таблица возвращается к корню, кроме случаев с `Irrevocable`.

#### Задержка таймера
Особые типы удержания, такие как `модификатор` и `часть аккорда`, откладывают таймер, и пока зажаты соответствующие клавиши, сброс таблицы и выполнение последнего действия не происходят.
...
### 📂 Модификаторы
Любой клавише _при удержании_ может быть назначен особый тип – модификатор. При назначении данного типа действия укажите числовое значение модификатора. Назначения из-под комбинируемых модификаторов получаются просто по сумме значений разных модификаторов (только разных).

#### Обратный переход
Каждый раз, когда вы доходите до последнего назначения в таблице переходов, вы возвращаетесь к начальной таблице. Но не в случае модификаторов – нажатия через них по умолчанию сохраняют вас на текущем уровне, позволяя обращаться к любому количеству назначений по данному пути. Даже нажатия несуществующих на уровне назначений не сбрасывают вас. Возврат произойдёт, когда вы отпустите все модификаторы. Но вы всё ещё можете уйти на более глубокий уровень, если у местных назначений будет непустая таблица переходов, и тогда ранее активный модификатор перестанет служить защитой от сброса.

#### Отображение
- Модификаторы отображаются в GUI с синей обводкой, текущие активные (зажатые) – с чёрной.
- Для переключения модификатора в GUI нажмите на него правой кнопкой мыши или удержите соответствующую физическую клавишу.
- Значение самих модификаторов также отображается в нижней части кнопки, как и прочие значения от удержания (если не задано собственное имя в gui).

> Нажатие модификаторов не влияет на уровень, они лишь меняют единое значение внутри одного уровня, которое будет учитываться для прочих назначений.
...
- **Функции** – бесконечное поле для предложений. Даже не рискну ничего придумывать для примера, это может быть действительно что-угодно. Но если оно полезно для вас, оно может быть полезным и для кого-то ещё.
