...
1. **Запустите** `tdfw.exe` или `tdfw.ahk`. 
   - При первом старте GUI:  
      - Смените по кругу все ваши установленные раскладки.
      - Используйте встроенные настройки для выбора формата раскладки, отображения GUI и длительности нажатий через 🔧 в правом нижнем углу основного окна.
...
### 🔄 Первый запуск
1. **Инициализация раскладок**: нажмите стандартное сочетание клавиш для смены раскладки (например, `Win + Space`) несколько раз, чтобы скрипт узнал, какие раскладки вы используете.
...
#### **Tap vs Hold** 
- Текст на кнопке — значения при нажатии (выше) и _удержании_ (ниже), если есть. 
- Цветная рамка — тип действия при удержании.
- Подчеркнутый текст — есть назначения на переходе от базового нажатия.
#### **Переходы**
...
- Что бы пойти по пути обратно | сбросить текущий путь, нажмите по одной из кнопок на нём.

### ⌨ Раскладки
Вы можете создавать назначения как для отдельной клавиатурной раскладки, так и для всех них сразу.
#### Выбор раскладки
Над клавишами-стрелками расположен выпадающий список с вашими раскладками (а также общими назначениями – *Global*), где вы можете выбирать, для какой раскладки добавлять назначения. Также в этот список будут добавлены раскладки, которые были найдены в ваших слоях, даже если эти слои неактивны. 
> В GUI вы, для меньшей путаницы, видите отдельно глобальные назначения, и отдельно назначения по каждой раскладке. Но в процессе работы основного скрипта эти назначения смешиваются – **к каждой раскладке добавляются глобальные назначения из раскладки Global**, с приоритетом для конкретных раскладок при перекрёстных назначениях. 
### 🌫️ Слои
Список слева отображает все найденные в `layers\` `*.json` слои. Изменением чекбокса устанавливайте активность слоя. Для манипуляций слоями используйте кнопки ниже списка – меняйте его относительный приоритет, изменяйте, удаляйте и добавляйте новые. 
...
#### Гармонизация 
Также список слоёв отображает назначения *по текущему пути* и количество переходов от него для каждого слоя, если они есть. Вы всегда знаете, что происходит на этом уровне на всех слоях.

### 🎨 Назначения и действия
Что бы добавить или изменить назначение – нажмите на соответствующую клавишу на вашей клавиатуре или нажмите на неё мышкой в интерфейсе. После нажатия, сверху справа от раскладки появится меню с панелями `Base` и `Hold`. `Base` отвечает за назначение при 
нажатии, `Hold` – при удержании. Нажмите на них, что бы ввести новые действия | назначения.

Для новых назначений есть несколько типов (_вы можете выбрать их в появившемся меню_):
...
- **AHK‑синтаксис**: команды вроде `{SC010}` или `+^{Left}`, для симуляции определённых нажатий, подробней смотрите на [сайте AHK](https://www.autohotkey.com/docs/v2/KeyList.htm).
...
#### Переход
После определения значения (базовое или от удержания) у него проверяется собственная таблица переходов. Если она есть, и она не пустая, текущая таблица переходов заменяется новой, и скрипт ожидает следующую клавишу по тому же значению таймера, без выполнения назначения текущего значения.
...
#### Возврат к началу
Каждый раз, когда вы доходите до последнего элемента цепочки, либо если цепочка прервалась таймером, таблица возвращается к начальному состоянию.
#### Задержка таймера
...
#### Ввод без возврата
Также возврат не происходит при вводе через `модификатор`. Пока зажат модификатор, вы можете выполнить любое количество нажатий для текущей таблицы, а возврат произойдёт только при отжатии модификатора.
...
### 📂 Модификаторы
Любой клавише _при удержании_ может быть назначен особый тип модификатора. При назначении, в поле типа выберете `Modifier`, после укажите числовое значение данного модификатора. Если вы зажимаете несколько назначенных модификаторов, их значения *складываются*.
#### Обратный переход
...
> Нажатие модификаторов не влияет на уровень | путь, они лишь меняют единое значение внутри одного уровня, которое будет учитываться для прочих нажатий.
